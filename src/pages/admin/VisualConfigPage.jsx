...
function Toast({ message, type, onNavigate }) {
  ...
  return (
    <div
      className={`fixed top-6 right-6 z-[100] flex items-center gap-3 px-5 py-3 rounded-sm shadow-lg text-white text-sm font-sans ${bg}`}
  ...
  return (
    <div className="flex items-center justify-between bg-white rounded-sm border border-[rgba(212,120,138,0.12)] px-5 py-3 mt-6">
      ...
      <div className="flex gap-3">
        <button
          onClick={onView}
          className="flex items-center gap-2 px-4 py-2 text-[0.65rem] font-sans font-medium tracking-widest uppercase border border-[rgba(212,120,138,0.2)] rounded-sm hover:bg-[#FDF0F3] transition-colors text-[#9A7480]"
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2 text-[0.65rem] font-sans font-semibold tracking-widest uppercase rounded-sm transition-all ${
            hasChanges
  ...
  return (
    <div className="min-h-screen bg-[#FDFAF9] p-6 lg:p-10">
      ...
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-10">
          <h1 className="font-display text-3xl font-light text-[#1A1118] mb-2">
        ...
        <div className="flex gap-1 mb-8 border-b border-[rgba(212,120,138,0.15)]">
          {[
            ...
            <button
              ...
              className={`px-5 py-3 text-xs font-sans font-medium tracking-widest uppercase transition-colors relative ${
                activeTab === tab.key
          ...
          <div className="space-y-8">
            ...
            {SECTIONS.map(({ key, label }) => (
              <div key={key} className="bg-white rounded-sm border border-[rgba(212,120,138,0.12)] p-6">
                <h3 className="font-display text-lg text-[#1A1118] mb-4">{label}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">Subir imagen</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">Predefinidas</label>
                      <div className="flex flex-wrap gap-2">
                        {PRESET_TEXTURES.map((tex) => (
                          <button key={tex.name}
                            onClick={() => setLocalTextures((prev) => ({ ...prev, [key]: { ...prev[key], url: tex.url } }))}
                            className="px-3 py-1.5 text-[0.65rem] font-sans border border-[rgba(212,120,138,0.2)] rounded-sm hover:bg-[#FDF0F3] transition-colors">
                            {tex.name}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">Fusión</label>
                      <select value={localTextures[key]?.blend || 'multiply'}
                        onChange={(e) => setLocalTextures((prev) => ({ ...prev, [key]: { ...prev[key], blend: e.target.value } }))}
                        className="w-full px-3 py-2 text-xs border border-[rgba(212,120,138,0.2)] rounded-sm bg-white text-[#1A1118] font-sans">
                    ...
                    {localTextures[key]?.url && (
                      <button onClick={() => setLocalTextures((prev) => ({ ...prev, [key]: { url: '', opacity: 0.04, blend: 'multiply' } }))}
                        className="text-xs text-[#E53935] hover:underline font-sans">Eliminar textura</button>
                    )}
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'decorations' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-sm border border-[rgba(212,120,138,0.12)] p-6">
              <h3 className="font-display text-lg text-[#1A1118] mb-4">Agregar decoración</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">Imagen</label>
                  <input type="file" accept="image/*" disabled={uploading}
                    onChange={(e) => { if (e.target.files[0]) handleUploadDecoration(e.target.files[0], 'global') }}
                    className="text-xs text-[#9A7480] file:mr-4 file:py-2 file:px-4 file:rounded-sm file:border-0 file:text-xs file:font-medium file:bg-[#FDF0F3] file:text-[#D4788A] hover:file:bg-[#F2C4CE]" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans">Sección</label>
            ...
            {localDecorations.length === 0 ? (
              <p className="text-xs text-[#9A7480] font-sans text-center py-8">No hay decoraciones</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div key={deco.id} className="bg-white rounded-sm border border-[rgba(212,120,138,0.12)] p-4">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 space-y-2">
                        <div>
                          <label className="block text-[0.6rem] text-[#9A7480] mb-1 font-sans">Opacidad: {Math.round((deco.opacity || 0.15) * 100)}%</label>
                        ...
                        <div>
                          <label className="block text-[0.6rem] text-[#9A7480] mb-1 font-sans">Rotación: {deco.rotation || 0}°</label>
                        ...
                        <div>
                          <label className="block text-[0.6rem] text-[#9A7480] mb-1 font-sans">Ancho: {deco.size?.width || '150px'}</label>
                        ...
                        <button onClick={() => setLocalDecorations((prev) => prev.filter((d) => d.id !== deco.id))}
                          className="text-xs text-[#E53935] hover:underline font-sans">Eliminar</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            {Object.entries(localTexts).map(([key, value]) => (
              <div key={key} className="bg-white rounded-sm border border-[rgba(212,120,138,0.12)] p-4">
                <label className="block text-xs font-medium text-[#4A3340] mb-2 font-sans capitalize">
                  {key.replace(/_/g, ' ')}
                </label>
                {value.length > 80 ? (
                  <textarea value={value} rows={3}
                    onChange={(e) => setLocalTexts((prev) => ({ ...prev, [key]: e.target.value }))}
                    className="w-full px-3 py-2 text-xs border border-[rgba(212,120,138,0.2)] rounded-sm bg-white text-[#1A1118] font-sans resize-none focus:outline-none focus:border-[#D4788A]" />
                ) : (
                  <input type="text" value={value}
                    onChange={(e) => setLocalTexts((prev) => ({ ...prev, [key]: e.target.value }))}
                    className="w-full px-3 py-2 text-xs border border-[rgba(212,120,138,0.2)] rounded-sm bg-white text-[#1A1118] font-sans focus:outline-none focus:border-[#D4788A]" />
                )}
              </div>
            ))}
          ...
          <div className="space-y-6">
            ...
            {Object.entries(localColors).map(([key, value]) => (
              <div key={key} className="bg-white rounded-sm border border-[rgba(212,120,138,0.12)] p-4 flex items-center gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-[#4A3340] font-sans capitalize">
                    {key === 'primary' ? 'Color principal' : key === 'accent' ? 'Color acento' : 'Fondo'}
                  </label>
                  <p className="text-[0.65rem] text-[#9A7480] font-sans">{value}</p>
                </div>
              </div>
            ))}
